...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $initials = generateInitialsFromFullName($row["user_name"]);
        echo '<tr id="user-row-' . htmlspecialchars($row["id"]) . '" 
        ...
        echo '<input class="form-check-input user-checkbox radius-4 border border-neutral-400" type="checkbox" value="' . htmlspecialchars($row["id"]) . '">';
        echo '</div>';
        echo '<span class="user-id">' . htmlspecialchars($initials) . '</span>';
        echo '</div>';
        echo '</td>';
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const apiCall = async (url, body) => {
                    ...
                    try {
                        ...
                        console.log('API Call - Response headers:', response.headers);
                ...
                document.getElementById('saveUserBtn').addEventListener('click', async function() {
                    ...
                    console.log('API URL:', 'wowdash-php/api-user-test.php');

                    const data = await apiCall('wowdash-php/api-user-test.php', userData);
